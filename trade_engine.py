...
class TradeEngine:
    ...
    def check_candle(self, instrument: str, ltp: float,
        ...
        1. Trail SAR SL
        2. Tier 0 : SAR reversed          → always exit immediately
        3. Tier 1 : ≥83% reversal pattern → exit immediately (same candle)
        4. Tier 2 : 80-82% pattern        → exit only when profit threshold met

        Exit direction contract:
          PE trade (bearish entry / red arrow) → exits on BULLISH reversal pattern
          CE trade (bullish entry / green arrow) → exits on BEARISH reversal pattern
          Direction matching is enforced by is_reversal_of() in app.py before
          this function is called — rev_pattern is already the opposite direction.
        """
        with self._lock:
            pos = self._positions.get(instrument)
            if not pos or not pos.is_open:
            ...
            pos.update_premium(ltp)
            pos.trail_sar(sar_value)

            # ── Current P&L ──────────────────────────────────────────────────
            pnl              = ltp - pos.entry_premium
            pnl_pct          = pnl / pos.entry_premium * 100 if pos.entry_premium else 0
            profit_threshold = config.INSTRUMENTS[instrument].get("profit_threshold", 0)

            # ── Tier 0: SAR reversal → always exit regardless of profit ──────
            if sar_reversed:
                return self._close(pos, instrument,
                                   f"Tier 0 exit: SAR reversed  new_sar={sar_value:.2f}")

            # ── Tier 1: ≥83% reversal pattern → immediate exit same candle ───
            # PE (red entry) exits on bullish pattern (green arrow).
            # CE (green entry) exits on bearish pattern (red arrow).
            if rev_pattern and rev_pattern in IMMEDIATE_EXIT_PATTERNS:
                logger.info(
                    f"{instrument}: Tier 1 immediate exit — '{rev_pattern}'  "
                    f"P&L={pnl:+.2f} ({pnl_pct:+.1f}%)"
                )
                return self._close(pos, instrument,
                                   f"Tier 1 exit: {rev_pattern} (≥83% immediate)")

            # ── Tier 2: 80-82% pattern → hold until profit threshold met ─────
            if rev_pattern:
                if not pos.pending_reversal:
                    # First occurrence — record it
                    pos.pending_reversal = rev_pattern
                    logger.info(
                        f"{instrument}: pending reversal '{rev_pattern}' detected — "
                        f"P&L={pnl:+.2f}  threshold={profit_threshold:.2f}"
                    )

                if pnl < profit_threshold:
                    logger.info(
                        f"{instrument}: holding — pending reversal "
                        f"'{pos.pending_reversal}' but profit {pnl:+.2f} "
                        f"< threshold {profit_threshold:.2f}"
                    )
                    return None

                # Profit threshold met — exit now
                reason = pos.pending_reversal
                logger.info(
                    f"{instrument}: Tier 2 exit — '{reason}'  "
                    f"P&L={pnl:+.2f} ({pnl_pct:+.1f}%)  threshold met"
                )
                return self._close(pos, instrument,
                                   f"Tier 2 exit: {reason} (profit threshold met)")
            else:
                # No reversal pattern this candle — clear stale pending flag
                pos.pending_reversal = ""
